...
test_that("model_donor throw an error for i = NULL and !=complete_rows", {
  expect_error(
    model_donor(df_XYZ_10_mis, is.na(df_XYZ_10_mis), NULL, list(selection = "partly_complete_rows")),
...
test_that("model_donor knn_complete_rows works", {
  ...
  expect_equal(
    ...
    model_donor(df_XYZ_10_mis, is.na(df_XYZ_10_mis), 2, list(selection = "knn_complete_rows", k = 3))
  )
})
...
test_that("model_donor partly_complete_rows works", {
  expect_equal(
    ...
    model_donor(df_XYZ_10_mis, is.na(df_XYZ_10_mis), i = 2, list(selection = "partly_complete_rows"))
  )

  expect_equal(
    structure(df_XYZ_10_mis[complete.cases(df_XYZ_10_mis), ], donor_selection = "partly_complete_rows"),
    model_donor(df_XYZ_10_mis, is.na(df_XYZ_10_mis), i = 7, list(selection = "partly_complete_rows"))
  )
})
...
test_that("model_donor knn_partly_complete_rows works", {
  ...
  expect_equal(
    ...
    model_donor(df_XYZ_10_mis, is.na(df_XYZ_10_mis), 4, list(selection = "knn_partly_complete_rows", k = 3))
  )
})

test_that("predict_donor average works", {
  donors_i_2 <- model_donor(df_XYZ_10_mis, is.na(df_XYZ_10_mis), 2, list(selection = "complete_rows"))
  expect_equal(
  ...
  donors_i_7 <- model_donor(df_XYZ_10_mis, is.na(df_XYZ_10_mis), 7, list(selection = "complete_rows"))
  expect_equal(
